    ...
    fun toTrack(trackId: Long): TrackSearch =
        TrackSearch.create(trackId).apply {
            ...
            if (userProgress != null) {
                library_id = stringToNumber(userProgress.reference)
                status = if (userProgress.status == "reading" && userProgress.rereads > 0) {
                    Hikka.REREADING
                } else {
                    toTrackStatus(userProgress.status)
                }
                last_chapter_read = userProgress.chapters.toFloat()
                score = userProgress.score.toFloat()
