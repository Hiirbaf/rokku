...
class MangaBakaApi(
    ...
    private val client = baseClient.newBuilder()
        .addInterceptor {
            it.request()
                ...
                .header(
                    "User-Agent",
                    "Rokku/${BuildConfig.VERSION_NAME} (${BuildConfig.APPLICATION_ID}) " +
                        "(Android) (https://github.com/thiago8rocha/Rokku)",
                ).build()
                .let(it::proceed)
        }.build()
    ...
    suspend fun addLibManga(track: Track): Track = withIOContext {
    ...
    suspend fun findLibManga(track: Track): Track? = withIOContext {
        with(json) {
            try {
                val url = "$LIBRARY_API_URL/${track.media_id}"
                val userData = authClient
                    .newCall(GET(url))
                    .awaitSuccess()
                    .parseAs<MangaBakaListResult>()
                    .data

                val additionalData = authClient
    ...
    suspend fun updateLibManga(track: Track): Track = withIOContext {
        val url = "$LIBRARY_API_URL/${track.media_id}"
        val body = buildJsonObject {
            put("state", track.toApiStatus())
            if (track.last_chapter_read > 0f) {
    ...
    suspend fun search(search: String): List<TrackSearch> = withIOContext {
        val url = "$API_BASE_URL/v1/series/search"
            .toUri()
            .buildUpon()
    ...
    suspend fun getMangaDetails(id: Long): TrackSearch? {
        return withIOContext {
            val url = "$API_BASE_URL/v1/series"
                .toUri()
                .buildUpon()
    ...
    suspend fun getCurrentUser(): MangaBakaUserProfile = withIOContext {
        with(json) {
            authClient
    ...
    suspend fun getAccessToken(code: String): MangaBakaOAuth = withIOContext {
        val formBody = FormBody
            .Builder()
            .add("client_id", CLIENT_ID)
            .add("code", code)
            .add("code_verifier", trackPreferences.mangabakaOAuthVerifier().get())
    ...
    fun verifyOAuthState(state: String): Boolean = state == trackPreferences.mangabakaOAuthState().get()
    ...
    companion object {
        ...
        fun authUrl(): Uri = "$OAUTH_URL/authorize"
            .toUri()
            .buildUpon()
        ...
        private fun getOAuthStateParam(): String {
            ...
            trackPreferences.mangabakaOAuthState().set(state)
            return state
        }
        ...
        private fun getPkceS256ChallengeCode(): String {
            ...
            trackPreferences.mangabakaOAuthVerifier().set(codes.codeVerifier)
