...
class Hikka(private val context: Context, id: Long) : TrackService(id) {
    ...
    override fun getStatusList() = listOf(PLAN_TO_READ, COMPLETED, ON_HOLD, READING, DROPPED, REREADING)
    ...
    override suspend fun refresh(track: Track): Track {
        ...
        track.score = readContent.score.toFloat()
        track.last_chapter_read = readContent.chapters.toFloat()
        // Hikka has no distinct "rereading" status remotely, just "reading" + a rereads counter.
        track.status = if (readContent.status == "reading" && readContent.rereads > 0) {
            REREADING
        } else {
            toTrackStatus(readContent.status)
        }
        track.started_reading_date = (readContent.startDate ?: 0L) * 1000
        track.finished_reading_date = (readContent.endDate ?: 0L) * 1000
